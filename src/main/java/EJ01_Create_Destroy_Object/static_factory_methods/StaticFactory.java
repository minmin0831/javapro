...
public class StaticFactory {
	...
	// 정적 팩토리와 생성자에는 똑같은 제약이 하나 있다. 선택적 매개변수가 많을 때, 적절히 대응하기 어렵다. 
	// 프로그래머들은 이럴 때 점층적 생성자 패턴(telescoping constructor pattern)을 사용하였으나, 이 방식도 매개변수가 많아지면 클라이언트 코드를 작성하거나 읽기 어렵다.  (14 페이지 참고)
	// 아울러 타입이 같은 매개 변수가 연달아 늘어서 있으면 찾기 어려운 버그로 이어질 수 있다. 
	// 이러한 점층적 생성자 패턴의 단점을 보완하고자 자바빈즈(java beans) 패턴을 사용할 수 있다. 각 매개변수를 하나씩 setter로 설정하기 때문에 코드가 길어지지만, 이전 방식의 단점을 찾아볼 수 없다. (16 페이지 참고)
	// 자바빈즈도 심각한 문제점을 지니고 있는데, 이는 자바빈즈 패턴에서 객체 하나를 만들려면 메서드를 여러 개 호출해야 하고, 객체가 완전히 생성되기 전까지는 일관성이 무너진 상태에 놓이게 된다. 
	// 점층적 생성자 패턴에서는 매개변수들이 유효한지를 생성자에서만 확인하면 일관성을 유지할 수 있었는데 그 장치가 완전히 사라진 것이다. 
	
	// 하지만 이에 대한 대안은 있다. 점층적 생성자 패턴의 '안전성'과 자바빈즈의 '가독성'을 겸비한 "빌터 패턴(Builder Pattern)"이다. 
